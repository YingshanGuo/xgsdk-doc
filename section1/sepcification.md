...
<li><a href="#shareConfiguration">社交分享配置</a></li>
...
- 游戏通过客户端代码传此地址（最高优先级，也可不选择此方式进行传递）
...
**注意：**如果充值回调地址已经在客户端进行传递，填在西瓜后台的充值回调地址即会失效；  
如果客户端未传递，填写了全局充值回调地址，部分渠道也填写了局部充值回调地址，对于这些渠道而言是取局部充值回调地址，其余渠道则取全局充值回调地址。
...
<img src="../section2/img/iOSpkg1.gif">
13.3.2 使用方法：  
...
### 16 调测
#### 16.1 调测功能说明
提供接口管理功能，用来检查客户端接口是否全部实现; 提供接口调用日志查询功能，在调侧过程中，根据用户名查询所有接口调用。

#### 16.2 调测功能使用

- 接口调用管理,在调测页面，可以查询哪些接口未调用，如图

 <img src="./image/commission-log.png"/>

- 查询调测日志详情,输入调测时使用的用户名关键字和调测时间，搜索符合条件的调测日志
 <img src="./image/commission-log1.png"/>

- 点击“查看详情”，查看交互流程及结果，如图

 <img src="./image/commission-log2.png"/>

- 选择流程节点，查看具体交互参数

 <img src="./image/comm3ission-log3.png"/>

<a id="shareConfiguration"></a>
### 17 社交分享配置

社交分享配置提供了对现行主流分享平台的支持。目前已支持的平台包括：

- 腾讯QQ
- 微信
- 新浪微博

要使用分享平台，我们应该首先打开分享开关，然后进行分享平台参数配置，接着打包工具将能够识别并为相应的渠道包添加分享功能模块。

- 分享开关分为两级：游戏级和平台级。请参考 16.1 打开/关闭分享

- 分享配置分为两种维度：产品维度和包维度。  
-- **产品维度**是一个游戏使用一套分享参数。目前腾讯QQ使用产品维度的配置方式；<br/>
-- **包维度**是一个游戏内的不同包名单独使用一套参数，目前微信和新浪微博使用此类型的配置方式；<br/>
具体配置信息请参考 16.2 配置分享平台参数

#### 17.1 打开/关闭分享

分享开关分两级，第一是游戏级别，总体控制游戏是否需要接入西瓜的分享模块，第二是分享平台级别，在游戏级别开关打开的情况下，控制具体需要接入哪个（哪些）分享平台。  
两级的社会化分享功能，默认都是关闭的。

##### 17.1.1 打开/关闭游戏级别分享开关
首先我们打开游戏级别的分享开关，打开基础配置 -> 游戏配置，点击【修改】按钮，弹出如下界面：

<img src="./image/game_config_share_config.png"/>

在界面中可以看到【分享配置】，默认【否】，可以根据需要选择【是】或【否】，然后点击确定即可。

##### 17.1.2 打开/关闭平台级别分享开关

打开游戏级别分享开关之后，还需要打开分享平台级的开关，分享功能才能最终起作用。  
操作步骤：<br/>
1. 从西瓜后台左侧选择【分享配置】；<br>
2. 右边将出现【腾讯QQ】、【新浪微博】、【微信】三个标签页，每个标签页的标题右侧有一个蓝色圆形按钮，亮起（蓝色）表示该平台分享处于开启状态，灰色表示该平台分享处于关闭状态。您可以通过点击改变其状态。

  具体示意图如下：<br/>
<img src="./image/share_enable_disable_platform.png"/>

#### 17.2 配置分享平台参数

如前所述，分享平台的配置方式分为两种维度。
...
##### 17.2.1 产品维度平台配置（腾讯QQ）

一个游戏要接入腾讯QQ的分享，只需要申请一套参数。并且这个参数只有一个即 **AppID**

<img src="./image/share_product_dimension_QQ.png"/>


##### 17.2.2 包维度平台配置（新浪微博和微信）

以新浪微博为例（微信也是一样的）。

新浪微博属于包维度，因为新浪平台的每一套参数配置都跟游戏包本身的包名有关（申请时需要提供包名），所以同一个游戏如果在不同的渠道使用不同的包名进行推广，就需要在新浪微博分享平台中，为每一个包名的游戏渠道包申请一套独立的参数。  
**注意：包维度配置需要输入包名，此包名请务必和游戏申请分享平台参数时候的包名完全一致！！**
